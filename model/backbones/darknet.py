...
def CBR(plane, kernel_size, stride=1, padding=0):
    return xnn.Sequential(xnn.Conv2d(plane, kernel_size, stride, padding, bias=False),
                          xnn.BatchNorm2d(),
...
class BasicBlock(xnn.Module):
    def __init__(self, plane):
        ...
        self.body = xnn.Sequential(CBR(plane // 2, kernel_size=1, stride=1, padding=0),
...
def darknet53_coco(fetch_feature=False):
    return Backbone([1, 2, 8, 8, 4], 'darknet53-coco', fetch_feature)
