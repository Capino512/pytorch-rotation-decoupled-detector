...
class BasicBlock(xnn.Module):
    ...
    def __init__(self, planes, stride=1, downsample=None, *args, **kwargs):
        ...
        self.body = xnn.Sequential(xnn.Conv2d(planes, 3, stride, 1, bias=False),
                                   xnn.BatchNorm2d(),
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, planes, stride=1, downsample=None, radix=1, cardinality=1, bottleneck_width=64, avd=False, avd_first=False, dilation=1, is_first=False):
...
class Backbone(xnn.Module):
    def __init__(self, block, layers, name=None, fetch_feature=False, radix=1, groups=1, bottleneck_width=64, dilated=False, dilation=1, deep_stem=False, stem_width=64, avg_down=False, avd=False, avd_first=False):
        ...
        self.head = xnn.Sequential(*head, xnn.BatchNorm2d(), nn.ReLU(inplace=True), nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
    ...
    def init(self):
        if self.name in weights:
            ...
            res = self.load_state_dict(weight, strict=False)
        else:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, is_first=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = xnn.Sequential(*down_layers)

        layers = []
        if dilation == 1 or dilation == 2:
            layers.append(block(planes, stride, downsample, self.radix, self.cardinality, self.bottleneck_width, self.avd, self.avd_first, 1, is_first))
        elif dilation == 4:
            layers.append(block(planes, stride, downsample, self.radix, self.cardinality, self.bottleneck_width, self.avd, self.avd_first, 2, is_first))
        else:
            raise RuntimeError("=> unknown dilation size: {}".format(dilation))

        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(planes, 1, None, self.radix, self.cardinality, self.bottleneck_width, self.avd, self.avd_first, dilation))
...
def resnest50(fetch_feature=False):
    return Backbone(Bottleneck, (3, 4, 6, 3), 'resnest50', fetch_feature, radix=2, deep_stem=True, stem_width=32, avg_down=True, avd=True, avd_first=False)


def resnest101(fetch_feature=False):
    return Backbone(Bottleneck, (3, 4, 23, 3), 'resnest101', fetch_feature, radix=2, deep_stem=True, stem_width=64, avg_down=True, avd=True, avd_first=False)


def resnest200(fetch_feature=False):
    return Backbone(Bottleneck, (3, 24, 36, 3), 'resnest200', fetch_feature, radix=2, deep_stem=True, stem_width=64, avg_down=True, avd=True, avd_first=False)


def resnest269(fetch_feature=False):
    return Backbone(Bottleneck, (3, 30, 48, 8), 'resnest269', fetch_feature, radix=2, deep_stem=True, stem_width=64, avg_down=True, avd=True, avd_first=False)
