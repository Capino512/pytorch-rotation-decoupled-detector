...
class FPN(xnn.Module):
    def __init__(self, depth, plane):
        super(FPN, self).__init__()
        self.link = xnn.ModuleList()
        self.fusion = xnn.ModuleList()
        for i in range(depth):
            self.link.append(xnn.Sequential(xnn.Conv2d(plane, 1, 1, 0, bias=False),
                                            xnn.BatchNorm2d()))
            if i != depth:
                self.fusion.append(xnn.Sequential(nn.ReLU(inplace=True),
    ...
    def forward(self, features):
        ...
        for i in range(len(features))[::-1]:
            if i != len(features) - 1:
                features[i] = self.fusion[i](features[i] + nn.Upsample(scale_factor=2)(features[i + 1]))
...
class PredictHead(xnn.Module):
    def __init__(self, plane, num_anchors, num_classes):
        ...
        self.body = xnn.Sequential(xnn.Conv2d(plane, 3, 1, 1, bias=False),
                                   xnn.BatchNorm2d(),
...
class DetPredict(xnn.Module):
    def __init__(self, depth, plane, num_anchors, num_classes, num_loc_params):
        ...
        self.heads_cls = xnn.ModuleList()
        self.heads_loc = xnn.ModuleList()
        for i in range(depth):
